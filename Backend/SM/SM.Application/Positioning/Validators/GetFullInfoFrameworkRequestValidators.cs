...
{
    public class GetFullInfoFrameworkRequestValidators
         : AbstractValidator<GetFullInfoFrameworkRequest>
    {
        public GetFullInfoFrameworkRequestValidators(IPermissionUserInteractor permissionUserInteractor)
        {
            RuleFor(x => x)
        ...
        expFramework.SubItems.Any(a => a == (long)ModulesSuItemsEnum.Framework))
                    {
                        context.AddFailure(
                          ...
                          $"Você não tem a tela enquadramento"));
                    }
